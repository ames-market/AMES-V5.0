...
def maximum_minimum_power_output_generators(model, minimum_power_output=None, maximum_power_output=None):
    model.MinimumPowerOutput = Param(model.Generators, initialize=minimum_power_output, within=NonNegativeReals,
                                     default=0.0)
    model.MaximumPowerOutput = Param(model.Generators, initialize=maximum_power_output, within=NonNegativeReals,
                                     default=0.0)
...
def start_up_shut_down_ramp_limits(model, scaled_start_up_ramp_limits=None, scaled_shut_down_ramp_limits=None):
    # SSU_j
    model.ScaledStartUpRampLimit = Param(model.Generators, within=NonNegativeReals,
                                         initialize=scaled_start_up_ramp_limits)
    # SSD_j
    model.ScaledShutDownRampLimit = Param(model.Generators, within=NonNegativeReals,
                                          initialize=scaled_shut_down_ramp_limits)


def minimum_up_minimum_down_time(model, scaled_minimum_up_time=None, scaled_minimum_down_time=None):
    # SUT_j
    model.ScaledMinimumUpTime = Param(model.Generators, within=NonNegativeIntegers, default=0,
                                      initialize=scaled_minimum_up_time)
    # SDT_j
    model.ScaledMinimumDownTime = Param(model.Generators, within=NonNegativeIntegers, default=0,
...
def initial_state(model, init_state=None,
    ...
    model.PowerGeneratedT0 = Param(model.Generators, within=NonNegativeReals, default=0,
                                   initialize=initial_power_generated)

    model.InitialTimePeriodsOnLine = Param(model.Generators, within=NonNegativeIntegers, default=0,
                                           initialize=initial_time_periods_online)

    model.InitialTimePeriodsOffLine = Param(model.Generators, within=NonNegativeIntegers, default=0,
                                            initialize=initial_time_periods_offline)
...
def hot_start_cold_start_costs(model,
    ...
    # SCST_j
    model.ScaledColdStartTime = Param(model.Generators, within=NonNegativeIntegers, default=0,
                                      initialize=scaled_cold_start_time)
    # HSC_j
    ...
    model.ShutdownCostCoefficient = Param(model.Generators, within=NonNegativeReals, default=0.0,
                                          initialize=shutdown_cost)
...
def _minimum_production_cost_fn(m, g):
    ...
    if len(m.CostPiecewisePoints[g]) > 1:
        ...
        # If there's only one piecewise point given, that point should be (MaxPower, MaxCost) --
        # i.e. the cost function is linear through (0,0),
...
def minimum_production_cost(model):
    model.MinimumProductionCost = Param(model.Generators, within=NonNegativeReals,
                                        initialize=_minimum_production_cost_fn, mutable=True)
...
def power_generation_piecewise_points_rule(m, g, t):
    min_production_cost = value(m.MinimumProductionCost[g])
