#This PSST file, originally due to Dheepak Krishnamurthy, has been modified by Swathi Battula to include Price Sensitive Load and Zonal data constraints and modified ramping constraints.
...
def calculate_total_demand(m, t, PriceSenLoadFlag=False):
    constraint = sum(m.NetFixedLoad[b,t] for b in m.Buses)
    ...
    constraint = m.TotalNetDemand[t] == constraint
...
def enforce_reserve_down_requirements_rule(m, t):
    ...
    constraint = constraint <= m.TotalNetDemand[t]
...
def enforce_zonal_reserve_down_requirement_rule(m, rz, t, 
    ...
    if PriceSenLoadFlag is False:
        constraint = constraint <= (1 - m.ZonalDownReservePercent[rz])* sum(m.NetFixedLoad[d, t] for d in m.DemandInReserveZone[rz])
    else:
        constraint = constraint <= (1 - m.ZonalDownReservePercent[rz]) * (sum(m.NetFixedLoad[d, t] for d in m.DemandInReserveZone[rz]) + sum(m.PSLoadDemand[l,t] for l in m.PriceSenLoadInReserveZone[rz]))
...
def enforce_zonal_reserve_up_requirement_rule(m, rz, t, 
    ...
    if PriceSenLoadFlag is False:
        constraint = constraint <= (1 - m.ZonalUpReservePercent[rz])* sum(m.NetFixedLoad[d, t] for d in m.DemandInReserveZone[rz])
    else:
        constraint = constraint <= (1 - m.ZonalUpReservePercent[rz]) * (sum(m.NetFixedLoad[d, t] for d in m.DemandInReserveZone[rz]) + sum(m.PSLoadDemand[l,t] for l in m.PriceSenLoadInReserveZone[rz]))
...
def constraint_reserves(model, PriceSenLoadFlag=False,
